...
var Module = {
	...
	UE4_indexedDBName: 'UE4_assetDatabase_HTML_TEstWork', // this should be an ascii ID string without special characters that is unique to the project that is being packaged
	UE4_indexedDBVersion: 202501270759, // Bump this number to invalidate existing IDB storages in browsers.
};
...
Module.arguments = ['../../../HTML_TEstWork/HTML_TEstWork.uproject','-stdout',];
...
function taskProgress(taskId, progress) {
	...
	if (progress && progress.total) {
		var percentage = (progress.current * 100 / progress.total).toFixed(0);
		var progressBar = document.querySelector('.progress-bar-custom');
		var loadText = document.getElementById('loadTasks');
		...
		if (loadText) {
			switch(taskId) {
				case TASK_DOWNLOADING:
					loadText.textContent = `Downloading... ${percentage}%`;
					break;
				case TASK_COMPILING:
					loadText.textContent = `Compiling... ${percentage}%`;
					break;
				case TASK_SHADERS:
					loadText.textContent = `Building shaders... ${percentage}%`;
					break;
				case TASK_MAIN:
					loadText.textContent = `Launching... ${percentage}%`;
					break;
			}
		}
	}
...
//	}
	...
	function withIndexedDB(db) {
		...
		// ----------------------------------------
		// MORE JS
		var dataJsDownload = fetchOrDownloadAndStore(db, Module.locateFile('HTML_TEstWork-HTML5-Shipping.data.js'));
...
/* // The following code would download and store the .data file as a Blob, which should be more efficient than loading an ArrayBuffer. However that seems to be buggy, so avoid it for now.
			fetchOrDownloadAndStore(db, Module.locateFile('HTML_TEstWork-HTML5-Shipping.data')).then(function(dataBlob) {
				return readBlobToArrayBuffer(dataBlob).then(function(dataArrayBuffer) {
					Module['preloadedPackages'] = {};
					Module['preloadedPackages'][Module.locateFile('HTML_TEstWork-HTML5-Shipping.data')] = dataArrayBuffer;
...
// Instead as a fallback, download as ArrayBuffer. (TODO: Figure out the bugs with the above, and switch to using that one instead)
			fetchOrDownloadAndStore(db, Module.locateFile('HTML_TEstWork-HTML5-Shipping.data'), 'arraybuffer').then(function(dataArrayBuffer) {
				Module['preloadedPackages'] = {};
				Module['preloadedPackages'][Module.locateFile('HTML_TEstWork-HTML5-Shipping.data')] = dataArrayBuffer;
				return dataJsDownload.then(addScriptToDom);
			});
